...
function checkWin() {
  ...
  if (shownLetters.length == guessableLetters.length) {
    ...
    document.querySelector('.intro-greeting').textContent = 'You won the game! Start a new game to try guessing a new phrase.';
  } else if (missed >= 5) {
    overlay.style.display = 'initial';
    overlay.classList = 'lose';
    document.querySelector('.intro-greeting').textContent = 'You lost! Please start a new game and try again.';
    missed = 0;
  }
}
